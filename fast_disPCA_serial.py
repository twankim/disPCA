...
class disPCA:
    ...
    def bamCompare(self,Ais,AiReps,i_row,i_ran,mode_exact):
        if mode_exact == 0: # Approximate distance
            vals = [spatial.distance.cosine(self.A[i_row,:],AiReps[i_r]) for i_r in i_ran]
    ...
    def disBAM(self,mode_exact=0,mode_sample=0,mode_norm=0):
        ...
        AiReps = [None] * d # Representative rows for each Ai
        ...
        for i in range(n):
            ...
            if mode_norm == 0: # Consider only frobenius norm
                ...
                if any(dRows[idx_ran] == 0):
                    ...
                    idx_sel = self.bamCompare(Ais,AiReps,i,idx_ran,mode_exact)
            ...
            if dRows[idx_sel] == 0:
                ...
                AiReps[idx_sel] = self.A[i,:] # Update repersentative row
                idx_dist[idx_sel] = np.array([i])
            else:
                Ais[idx_sel] = np.concatenate((Ais[idx_sel],self.A[i,:][None,:]),axis=0)
                # AiReps[idx_sel] = self.A[i,:] # Update repersentative row as previously assigned row
                AiReps[idx_sel] = (AiReps[idx_sel]*dRows[idx_sel]+self.A[i,:])/float(dRows[idx_sel]+1) # Update repersentative row as average of stored rows
